...
export function AdminPage() {
  ...
  const tabBtn = (active: boolean) =>
    `rounded-xl py-2 px-5 text-sm font-medium transition-all cursor-pointer border-none ${
      active ? 'bg-white/80 text-[#1d1d1f] shadow-sm' : 'text-[#6e6e73] bg-transparent hover:text-[#1d1d1f]'
    }`
    ...
    <div className="min-h-screen px-4 py-8 pb-16">
      <div className="max-w-5xl mx-auto">
        <header className="flex items-center justify-between mb-8">
          <h1 className="text-2xl font-semibold text-[#1d1d1f] tracking-tight m-0">Panou admin</h1>
          <button
            onClick={signOut}
        ...
        <div className="flex gap-1 mb-7 glass rounded-2xl p-1 w-fit">
          <button className={tabBtn(tab === 'appointments')} onClick={() => setTab('appointments')}>
            Programări
          </button>
          <button className={tabBtn(tab === 'slots')} onClick={() => setTab('slots')}>
            Disponibilitate
          </button>
          <button className={tabBtn(tab === 'services')} onClick={() => setTab('services')}>
            Servicii
          </button>
      </div>
      ...
      {tab === 'appointments' && (
        <>
          {error && <p className="text-sm text-red-500 mb-4">Eroare: {error}</p>}
          {loading && <p className="text-sm text-[#6e6e73] mb-4">Se încarcă programările...</p>}
        ...
        <tbody>
          {items.map((a) => (
            <tr key={a.id} className={`border-b border-white/30 last:border-0 hover:bg-white/20 transition-colors ${a.status === 'cancelled' ? 'opacity-40' : ''}`}>
              ...
              <td className="px-4 py-3 text-[#1d1d1f] whitespace-nowrap">
                {a.available_slots
                  ? `${formatTime(a.available_slots.start_time)} – ${formatTime(a.available_slots.end_time)}`
                  : '—'}
              </td>
